...
class SettingsStore : AutoCloseable {
    ...
    /**
     * Сохраняет переданные нормализованные значения несекретных настроек и при
     * необходимости обновляет [secrets]. Полная валидация конфигурации выполняется
     * перед запуском мониторинга или проверкой подключений.
     *
